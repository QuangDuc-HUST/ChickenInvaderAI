    ...
    if step % 3 == 0:
        try:
            laying_invader = sorted(np.random.choice(
                range(len(space.invaders)), 3, replace=False))
            # if step % 3 == 0:
...
def check_collision(space: 'Space'):
    ...
    for egg in space.eggs.copy():
        if space.spaceship.collide(egg):
            print(
                f'collision occurs at x= {space.spaceship.x} , y ={space.spaceship.y}')
            return True
    return False
...
def to_json(figure: 'np.ndarray', all_step: dict, step: int):
    '''
    Store the figure as a json object in a dictionary with key=step
    '''
